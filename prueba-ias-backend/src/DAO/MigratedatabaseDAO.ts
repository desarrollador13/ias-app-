...
export class MigratedatabaseDAO {
	...
	constructor(
		...
		this.hostNameLocalD=`postgresql://postgres:admin@localhost/app`
    ...
    **/
    ...
    async eliminardatabase():Promise<object|any>{
    	const connectionString = this.hostNameLocal //this.hostDocker
			const pool = new Pool({
	...
	async createTables():Promise<object|any> {
		const connectionString = this.hostNameLocalD  //`postgresql://postgres:admin@localhost/ban_colombia` //this.hostDocker
		const pool = new Pool({
		  connectionString: connectionString,
		})
		let resc:object|any={}
	...
	async  insertTable():Promise<object|any> {
		...
		const connectionString =  this.hostNameLocalD //`postgresql://postgres:admin@localhost/ban_colombia`//this.hostdatabase //this.hostDocker
		const pool = new Pool({
		  connectionString: connectionString,
		})
		...
		try{
			...
			return {code:200, msg:'migrate exitoso', rows: resIu5.rows}
			pool.end()	
		}catch(error){
			console.log(error,'error migrar')
